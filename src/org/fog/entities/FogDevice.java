...
public class FogDevice extends PowerDatacenter {
	...
	private void updateModuleInstanceCount(SimEvent ev) {
		...
		System.out.println(getCapitalizedName() + " Creating " + config.getInstanceCount() + " instances of module "+config.getModule().getName());
	}
	...
	private void sendPeriodicTuple(SimEvent ev) {
		...
		for(int i = 0;i<((edge.getDirection()==Tuple.UP)?instanceCount:1);i++){
			...
			DataPacket nullData = new DataPacket("null".getBytes());
			nullData.setEncryptionType(EncryptionType.NONE);
			tuple.setDataPacket(nullData);
	...
	//Method to decide if key should be stored or sent on
	public void processKey(SimEvent ev){

		//Process a key arriving
		Tuple tuple = (Tuple) ev.getData();
		//If the exchange type is asymmetric
		if(tuple.getKeyExchangeLocation() == CommunicatedDevice.ENDPOINT && CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.EXCHANGEAYSMMETRIC) {
			actuatorIdPublicKey.put(tuple.getActuatorId(), new String(tuple.getDataPacket().getData()));
			Cryptography.generateAsymmetricKeys();
			devicePrivateKey = Cryptography.getAsymmetricPrivateKey();
			devicePublicKey = Cryptography.getAsymmetricPublicKey();
			send(ev.getSource(), CloudSim.getMinTimeBetweenEvents(), FogEvents.KEY_ARRIVAL, devicePublicKey);
		}
		//If the exchange type is symmetric
		else if (tuple.getKeyExchangeLocation() == CommunicatedDevice.ENDPOINT && CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.EXCHANGESYMMETRIC){
			actuatorIdPublicKey.put(tuple.getActuatorId(),new String(tuple.getDataPacket().getData()));
	...
	protected void sendTupleToActuator(Tuple tuple){
		...
		if(CryptographyConfig.getCryptographyEnabled()) {
			//If cryptography has been enabled then encrypt the data being sent to the actuator
			DataPacket tupleData = new DataPacket(generateActuatorData());

			if (encryptionKey == null && CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.EXCHANGEAYSMMETRIC) {
				encryptionKey = actuatorIdPublicKey.get(tuple.getActuatorId());
			}
			else if (encryptionKey == null && CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.EXCHANGESYMMETRIC) {
				encryptionKey = actuatorIdPublicKey.get(tuple.getActuatorId());
			}
			else if (encryptionKey == null && CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.STOREDASYMMETRIC) {
				encryptionKey = Cryptography.getStoredAsymmetricPublicKey();
			}
			else if (encryptionKey == null && CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.STOREDSYMMETRIC) {
				encryptionKey = Cryptography.getStoredSymmetricKey();
			}
			if(CryptographyConfig.getEncryptionType() == EncryptionType.RSA){
				if(CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.STOREDASYMMETRIC){
					...
					tupleData.setDigitalSignature(Cryptography.sign(tupleData.getData(),devicePrivateKey));
				}
			}
	...
	Code added by Ben Farmer
	 */
	//Method to generate data for the actuator
	protected byte[] generateActuatorData(){
		long leftLimit = 1L;
		long rightLimit = 10L;
	...
	HashMap<Integer, String> sensorIdPublicKey = new HashMap<Integer,String>(); // Hashmap containing the key for the cloud
	HashMap<Integer, String> sensorIdPrivateKey = new HashMap<Integer, String>(); //Contains the device private key when decrypting information from the sensor
	HashMap<Integer, String> actuatorIdPublicKey = new HashMap<Integer, String>(); //Contains the


	protected void keyRequest(SimEvent ev){ //Process the key request from different devices
		Tuple tuple = (Tuple)ev.getData();

		System.out.println(getCapitalizedName() + " : Has received a key request");
		//If the key exchange type is asymmetric
		if(CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.EXCHANGEAYSMMETRIC){

			//Store the sensors public key along with its id
			sensorIdPublicKey.put(tuple.getSourceDeviceId(),new String(tuple.getDataPacket().getData()));

			//If the key request is for the fog device send back key
			if(tuple.getKeyExchangeLocation() == CommunicatedDevice.ENDPOINT){
				//Generate the key pair
				Cryptography.generateAsymmetricKeys();

				//Send back the public key of this device
				send(tuple.getSourceDeviceId(),1,FogEvents.KEY_ARRIVAL, Cryptography.getAsymmetricPublicKey());

				//Store the key in the sensor/actuator map along with the id
				if(tuple.getTupleType() == "ACTUATORKEYREQUEST"){
					actuatorIdPublicKey.put(tuple.getActuatorId(),Cryptography.getAsymmetricPrivateKey());
				}
				else{
					sensorIdPrivateKey.put(tuple.getSourceDeviceId(),Cryptography.getAsymmetricPrivateKey());
				}

			}

			//If the key request is cloud and current location is cloud send key back
			else if(getName().equals("cloud") && tuple.getKeyExchangeLocation() == CommunicatedDevice.CLOUD){
				//Generate a keypair
				Cryptography.generateAsymmetricKeys();


				//Send the devices public key back to the devive
				send(tuple.getSourceDeviceId(),1,FogEvents.KEY_ARRIVAL,Cryptography.getAsymmetricPublicKey());
			...
			//If key request is not this location send up until it reaches the desired location
			else if(tuple.getDirection() == Tuple.UP) {
				send(parentId,1,FogEvents.KEY_REQUEST, tuple);
			}
		}

		//If exchanging keys for symmetric encryption
		else if (CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.EXCHANGESYMMETRIC) {
			//If the key request is for the fog device send back key
			if(tuple.getKeyExchangeLocation() == CommunicatedDevice.ENDPOINT ){
				Cryptography.generateSymmetricKey();
				String encryptionKeyTemp = Cryptography.getStoredSymmetricKey();
				byte[] encryptedKey = (Cryptography.Encrypt(encryptionKeyTemp.getBytes(),new String(tuple.getDataPacket().getData()),EncryptionType.RSA));
				if(tuple.getTupleType() == "ACTUATORKEYREQUEST"){
					actuatorIdPublicKey.put(tuple.getActuatorId(),encryptionKeyTemp);
				}
				else{
					sensorIdPrivateKey.put(tuple.getSourceDeviceId(),encryptionKeyTemp);
				}
				send(tuple.getSourceDeviceId(),1,FogEvents.KEY_ARRIVAL,encryptedKey);
			}
			//If the key request is cloud and current location is cloud send key back
			else if(getName().equals("cloud") && tuple.getKeyExchangeLocation() == CommunicatedDevice.CLOUD){
				Cryptography.generateSymmetricKey();
		...
		System.out.println(getCapitalizedName() + " : Has sent a key");
	...
	protected void processTupleArrival(SimEvent ev){
		...
		if(getName().equals("cloud")){
			...
			Code added by Ben Farmer
			 */
		//Only run cryptographic methods when it is enabled
		if(CryptographyConfig.getCryptographyEnabled()) {

			//Check what type of key exchange is being done, it its stored then get the key and set it
			if(encryptionKey == null && CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.STOREDSYMMETRIC){
				decryptionKey = Cryptography.getStoredSymmetricKey();
			}
			//Check if the keys are null set them with the public and private key
			else if(encryptionKey == null && CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.STOREDASYMMETRIC){
				decryptionKey = Cryptography.getStoredAsymmetricPrivateKey(); //Get clouds private key to decrypt
			}
			//Set the correct encryption key if its exchange aysmmetric
			else if(CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.EXCHANGEAYSMMETRIC || CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.EXCHANGESYMMETRIC){
				decryptionKey = sensorIdPrivateKey.get(tuple.getDataPacket().getOriginSource());
			}

			//If the decrpt location is in endpoint and that is the current location, decrypt the data.
			if(tuple.getDataPacket().encryptionType != EncryptionType.NONE && (tuple.getDataPacket().getEncDecLocations() == CommunicatedDevice.ENDPOINT)){
				//Decrypt the packet
				...
				//If the encryption type is RSA then check the digital signature.
				if(tuple.getDataPacket().getEncryptionType() == EncryptionType.RSA) {
					if(CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.STOREDASYMMETRIC){
						if (Cryptography.verify(tuple.getDataPacket().getData(), tuple.getDataPacket().getDigitalSignature(), Cryptography.getStoredAsymmetricPublicKey())){
							System.out.println(getCapitalizedName() + " : Has verified the digital signature");
						} else {
							System.out.println(getCapitalizedName() + " : Digital signature not verified");
						}
					}
					else{
						if (Cryptography.verify(tuple.getDataPacket().getData(), tuple.getDataPacket().getDigitalSignature(), sensorIdPublicKey.get(tuple.getDataPacket().getOriginSource()))) {
							System.out.println(getCapitalizedName() + " : Has verified the digital signature");
						} else {
							System.out.println(getCapitalizedName() + " : Cannot verify the digital signature");
						}
					}

				}
				//Mark that the tuple has been decrypted
				tuple.getDataPacket().setEncryptionType(EncryptionType.NONE);
				System.out.println(getCapitalizedName() + " : Has decrypred  " + new String(tuple.getDataPacket().getData()));

			}
			else if (tuple.getDataPacket().encryptionType != EncryptionType.NONE && (getName().equals("cloud") && tuple.getDataPacket().getEncDecLocations() == CommunicatedDevice.CLOUD)) {
				//Decrypt the packet as soon as it enters the cloud
				tuple.getDataPacket().setData(Cryptography.Decrypt(tuple.getDataPacket().getData(), decryptionKey, tuple.getDataPacket().encryptionType));
				if(tuple.getDataPacket().getEncryptionType() == EncryptionType.RSA) {
					if(CryptographyConfig.getKeyExchangeType() == KeyExchangeTypes.STOREDASYMMETRIC){
						if (Cryptography.verify(tuple.getDataPacket().getData(), tuple.getDataPacket().getDigitalSignature(), Cryptography.getStoredAsymmetricPublicKey())) {
							System.out.println("Digital signature verified by " + getName());
						}
						else {
							System.out.println(getCapitalizedName() + " : Digital signature not verified");
						}
				...
				System.out.println(getCapitalizedName() + " : Has decrypted Tuple to - " + new String(tuple.getDataPacket().getData()));
